...
@Mod(MoltenSmith.MODID)
public class MoltenSmith {
    // Define mod id in a common place for everything to reference
    public static final String MODID = "moltensmith";
    // Directly reference a slf4j logger
    public static final Logger LOGGER = LogUtils.getLogger();
    // Create a Deferred Register to hold Blocks which will all be registered under the "moltensmith" namespace
    public static final DeferredRegister.Blocks BLOCKS = DeferredRegister.createBlocks(MODID);
    // Create a Deferred Register to hold Items which will all be registered under the "moltensmith" namespace
    public static final DeferredRegister.Items ITEMS = DeferredRegister.createItems(MODID);
    // Create a Deferred Register to hold CreativeModeTabs which will all be registered under the "moltensmith" namespace
    public static final DeferredRegister<CreativeModeTab> CREATIVE_MODE_TABS = DeferredRegister.create(Registries.CREATIVE_MODE_TAB, MODID);

    // Creates a new Block with the id "moltensmith:example_block", combining the namespace and path
    public static final DeferredBlock<Block> EXAMPLE_BLOCK = BLOCKS.registerSimpleBlock("example_block", BlockBehaviour.Properties.of().mapColor(MapColor.STONE));
    // Creates a new BlockItem with the id "moltensmith:example_block", combining the namespace and path
    public static final DeferredItem<BlockItem> EXAMPLE_BLOCK_ITEM = ITEMS.registerSimpleBlockItem("example_block", EXAMPLE_BLOCK);

    // Creates a new food item with the id "moltensmith:example_id", nutrition 1 and saturation 2
    public static final DeferredItem<Item> EXAMPLE_ITEM = ITEMS.registerSimpleItem("example_item", new Item.Properties().food(new FoodProperties.Builder()
            .alwaysEdible().nutrition(1).saturationModifier(2f).build()));

    // Creates a creative tab with the id "moltensmith:moltensmith_tab" for the example item, that is placed after the combat tab
    ...
    private void commonSetup(FMLCommonSetupEvent event) {
        // Initialize MoltenSmith systems
        LOGGER.info("MoltenSmith - Initializing weapon forging systems...");
        
        // Initialize material registry
        event.enqueueWork(() -> {
            MaterialRegistry.init();
            LOGGER.info("Material system initialized with {} materials", MaterialRegistry.size());
        });
    ...
    public void onServerStarting(ServerStartingEvent event) {
        ...
        LOGGER.info("MoltenSmith - Server starting, loading materials and recipes...");
    }
}
